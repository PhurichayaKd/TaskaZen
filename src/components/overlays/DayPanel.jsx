      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Content Area */}
            <div className="flex-1 overflow-y-auto bg-zinc-50/30 custom-scrollbar relative">
              <div className="p-6 pb-24 flex flex-col gap-8">
                    ...
                    {categoriesConfig.map((cat) => (
                      <button
                        ...
                        className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-all ${
                          category === cat.id ? 'bg-indigo-50 text-indigo-700 border border-indigo-200 shadow-sm' : 'bg-white text-zinc-600 border border-zinc-200 hover:bg-zinc-50'
                        }`}
                      >
                ...
                <div className="space-y-3">
                  <label className="text-sm font-medium text-zinc-900">บันทึกความคิดและโน้ต</label>
                  <textarea 
                    value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="วันนี้มีอะไรในหัวบ้าง จดไว้เลย..."
                    className="w-full h-24 p-3 text-sm border border-zinc-200 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all resize-none shadow-sm"
                  />
                  <div className="flex items-center justify-between mt-2">
                    <Button type="button" variant="outline" size="sm" onClick={handleAITaskBreakdown} disabled={isGeneratingTasks} className="text-indigo-600 border-indigo-200 hover:bg-indigo-50 bg-indigo-50/50">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-t border-zinc-200 pt-6">
                    ...
                    <Badge className="bg-zinc-100 text-zinc-800 border border-zinc-200">{tasks.length} รายการ</Badge>
                  </div>
                  
                  {/* Add Task Form */}
                  <form onSubmit={handleAddTask} className={`flex flex-col gap-3 p-4 border rounded-xl shadow-sm transition-all ${editingTask ? 'bg-indigo-50/50 border-indigo-200' : 'bg-white border-zinc-200'}`}>
                    {editingTask && (
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-bold text-indigo-600 flex items-center"><Settings2 className="w-3 h-3 mr-1" /> กำลังแก้ไขงาน</span>
                        <button type="button" onClick={cancelEditing} className="text-xs text-zinc-400 hover:text-zinc-600">ยกเลิก</button>
                      </div>
                    )}
                    <div className="flex gap-2">
                      <input 
                        type="time" value={newTaskTime} onChange={(e) => setNewTaskTime(e.target.value)}
                        className="w-28 p-2 text-sm border border-zinc-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-600 bg-zinc-50"
                      />
                      <input 
                        type="text" placeholder="เพิ่มงานใหม่..." value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} required
                        className="flex-1 p-2 text-sm border border-zinc-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-600"
                      />
                    </div>
                    
                    <div className="flex justify-between items-center">
                      <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="text-xs text-zinc-500 hover:text-indigo-600 flex items-center font-medium transition-colors">
                        <Settings2 className="w-3 h-3 mr-1" /> {showAdvanced ? 'ซ่อนการตั้งค่า' : 'ตั้งค่าเพิ่มเติม (สี, ความยาก, ระดับพลังงาน)'}
                      </button>
                      <Button type="submit" size="sm" className={`h-8 ${editingTask ? 'bg-indigo-600 hover:bg-indigo-700' : ''}`}>
                    ...
                    <AnimatePresence>
                      {showAdvanced && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                           <div className="pt-4 mt-2 border-t border-zinc-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            
                            <div className="space-y-1.5">
                              <label className="text-[10px] font-bold text-zinc-500 uppercase flex items-center"><Flag className="w-3 h-3 mr-1" /> ความสำคัญ</label>
                              <div className="flex gap-1 bg-zinc-50 p-1 rounded-md border border-zinc-200">
                                {[
                                  ...
                                  <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-medium rounded border transition-all ${newTaskPriority === opt.id ? opt.activeClass : 'border-transparent text-zinc-500 hover:bg-zinc-200/50'}`}
                                  >
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[10px] font-bold text-zinc-500 uppercase flex items-center"><Target className="w-3 h-3 mr-1" /> ความยาก</label>
                              <div className="flex gap-1 bg-zinc-50 p-1 rounded-md border border-zinc-200">
                                {[
                                  ...
                                  <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-medium rounded border transition-all ${newTaskDifficulty === opt.id ? opt.activeClass : 'border-transparent text-zinc-500 hover:bg-zinc-200/50'}`}
                                  >
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[10px] font-bold text-zinc-500 uppercase flex items-center"><Zap className="w-3 h-3 mr-1" /> ระดับพลังงาน (1-5)</label>
                              <div className="flex gap-1 bg-zinc-50 p-1 rounded-md border border-zinc-200">
                                {['1', '2', '3', '4', '5'].map(lvl => (
                                  <button
                                    key={lvl} type="button" onClick={() => setNewTaskLevel(lvl)}
                                    className={`flex-1 flex items-center justify-center py-1.5 text-xs font-bold rounded border transition-all ${newTaskLevel === lvl ? 'bg-white text-indigo-600 shadow-sm border-indigo-200' : 'border-transparent text-zinc-400 hover:bg-zinc-200/50 hover:text-zinc-600'}`}
                                  >
                                    {lvl}
                                  </button>
                                ))}
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[10px] font-bold text-zinc-500 uppercase flex items-center"><Palette className="w-3 h-3 mr-1" /> สีแท็ก</label>
                              <div className="flex items-center gap-2 h-[34px] px-2 bg-zinc-50 rounded-md border border-zinc-200 w-full justify-between">
                                {Object.keys(colorMap).map(colorKey => (
                                  <button 
                                    ...
                                    className={`w-5 h-5 rounded-full flex-shrink-0 transition-all ${colorMap[colorKey].main} ${newTaskColor === colorKey ? 'ring-2 ring-offset-1 ring-zinc-400 scale-110' : 'opacity-60 hover:opacity-100 hover:scale-110'}`}
                                  />
                                ))}
                              </div>
                            </div>
                  ...
                  <div className="space-y-6 mt-4">
                    {groupedTasks.length === 0 ? (
                      <p className="text-sm text-zinc-400 text-center py-8 border border-dashed border-zinc-200 rounded-xl bg-white">ไม่มีงานที่กำหนดไว้สำหรับวันนี้</p>
                    ) : (
                                  ...
                                  <div className="flex items-start justify-between pl-2">
                                    <div className="flex items-start gap-3 w-full">
                                      <button 
                                        ...
                                        className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${task.completed ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-zinc-300 hover:border-indigo-600'}`}
                                      >
                                      ...
                                      <div className="flex-1 w-full min-w-0">
                                        <div className="flex justify-between items-start gap-2 w-full">
                                          <p 
                                            ...
                                            className={`text-sm font-bold flex-1 cursor-pointer hover:bg-zinc-100 px-1 -mx-1 rounded transition-colors ${task.completed ? 'text-zinc-400 line-through' : 'text-zinc-900'}`}
                                          >
                                            {task.text}
                                          </p>
